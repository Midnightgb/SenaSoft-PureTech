<template>
  <div class="flex flex-col h-screen w-screen bg-green-50 dark:bg-gray-900">
    <header class="p-4 bg-green-500 dark:bg-green-700 text-white flex items-center">
      <h1 class="text-xl font-bold">Recycling Submission</h1>
    </header>
    <main class="flex-1 overflow-auto p-4">
      <form @submit.prevent="submitForm" class="bg-white dark:bg-gray-800 rounded-lg p-6 shadow">
        <h2 class="text-lg font-semibold mb-4 dark:text-white">Submit Recycling Data</h2>

        <div v-if="errorMessage" class="mb-4 p-2 bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-200 rounded">
          {{ errorMessage }}
        </div>

        <div v-if="successMessage" class="mb-4 p-2 bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-200 rounded">
          {{ successMessage }}
        </div>
        ...
        <div class="mb-6 hidden">
          <label for="recycling_point_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Recycling Point ID</label>
          <input
            type="number"
        ...
        <div class="mb-4">
          <p class="text-sm font-medium text-gray-700 dark:text-gray-300">Estimated Points: {{ calculatePoints }}</p>
        </div>

        <div class="flex items-center justify-end">
          <button
            type="submit"
            class="bg-green-500 hover:bg-green-600 dark:bg-green-600 dark:hover:bg-green-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
          >
